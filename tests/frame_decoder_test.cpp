...
TEST(FrameDecoderTest, ParsesPartialFrames) {
  ...
  EXPECT_EQ(decoder.push(chunk2, sizeof(chunk2), out),
...
TEST(FrameDecoderTest, RejectsFrameExceedingCeilingGuard) {
  ...
  // Send a chunk of 135 bytes (larger than 128 + 4096 is not needed since our buffer ceiling limit
  // check is: buf_.size() - read_idx_ + len > max_frame_size_ + 4096 With max_frame_size_ = 128,
  // the limit is 128 + 4096 = 4224 bytes. Let's push data larger than 4224 bytes.

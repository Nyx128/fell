...
namespace fell::storage {
  ...
  struct StorageOptions {
    ...
    uint32_t batch_wait_us = 1000;        ///< Queue latency/gather deadline window in microseconds (default 1ms)

    // --- Flushing / Durability Policies ---
    uint32_t flush_every_records = 8192; ///< Force fsync/flush if un-fsynced count is met
    uint32_t flush_every_ms = 25;        ///< Force fsync/flush if age of oldest batch reaches this in milliseconds
  };
